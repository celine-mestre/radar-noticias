...
| `remarcar.py` | **Regras de hoje aplicadas ao arquivo de ontem.** A marcação acontece na recolha: uma expressão acrescentada hoje só marcaria daí para a frente, e uma removida continuaria a sustentar marcações antigas. Este passo relê cada notícia arquivada, reaplica as regras atuais ao título e ao resumo, atualiza as palavras-chave e retira as marcações que já não se sustentam. Corre-se à mão, sempre com `--ensaio` primeiro. Tem um modo `--completo`, que reclassifica também os artigos hoje sem área, **a usar com cuidado**: o arquivo integral só existe desde 10 de agosto de 2026, pelo que uma reclassificação total encontraria material novo apenas nos dias recentes e criaria um degrau artificial na série. |
